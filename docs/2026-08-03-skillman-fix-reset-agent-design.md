...
## 追加:重置不再清空 skill-backups(2026-08-03)

用户要求:reset 不再清空 `~/.skillman/skill-backups/`,该目录作为操作安全网保留,出 bug 时可手动找回文件。

改动:

- `lifecycle.rs::reset_all` 移除 `clear_dir_contents(backups_dir())` 调用,只恢复 skill + 清空 projects 表;
- `fsutil.rs` 删除因此变成死代码的 `clear_dir_contents` 及其测试;
- 设置页「重置数据」描述与确认弹窗文案同步更新(明确备份会保留);
- 新增回归测试 `reset_all_keeps_backups_dir`:导入(产生 preimport 备份)→ 重置 → 断言备份目录内容原样保留。
